	        ...
	        <?php if( get_theme_mod( 'florist_flower_shop_grid_postdate',true) == 1 || get_theme_mod( 'florist_flower_shop_grid_author',true) == 1 || get_theme_mod( 'florist_flower_shop_grid_comments',true) == 1) { ?>
		        <div class="post-info p-2 my-3">
		          ...
		          <?php if(get_theme_mod('florist_flower_shop_grid_author',true)==1){ ?>
		            <span><?php echo esc_html(get_theme_mod('florist_flower_shop_grid_post_meta_field_separator', '|'));?></span> <i class="<?php echo esc_attr(get_theme_mod('florist_flower_shop_grid_author_icon','fas fa-user')); ?>"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span>
		          <?php } ?>

		          <?php if(get_theme_mod('florist_flower_shop_grid_comments',true)==1){ ?>
		            <span><?php echo esc_html(get_theme_mod('florist_flower_shop_grid_post_meta_field_separator', '|'));?></span> <i class="<?php echo esc_attr(get_theme_mod('florist_flower_shop_grid_comments_icon','fa fa-comments')); ?>" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'florist-flower-shop'), __('0 Comments', 'florist-flower-shop'), __('% Comments', 'florist-flower-shop') ); ?></span>
		          <?php } ?>
		        </div>
		    <?php } ?>
	        <div class="new-text">
	        	<p>
			        <?php $florist_flower_shop_excerpt = get_the_excerpt(); echo esc_html( florist_flower_shop_string_limit_words( $florist_flower_shop_excerpt, esc_attr(get_theme_mod('florist_flower_shop_related_posts_excerpt_number','30')))); ?> <?php echo esc_html( get_theme_mod('florist_flower_shop_excerpt_suffix','') ); ?>
	        	</p>
	        </div>
